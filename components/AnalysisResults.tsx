  ...
  return (
    <div className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg p-4 shadow-lg">
      ...
      <div className="flex overflow-x-auto space-x-4 p-2 -m-2">
        {/* FIX: Replaced 'analyzedMap' with 'mapImage' to match the AnalysisRecord type. */}
        {analysisHistory.map(({ revision, mapImage, leakMap, passed }) => (
          <div key={revision} className="flex-shrink-0 w-48 text-center group">
            <div className="relative">
                <button
                    onClick={() => onImageClick(mapImage, leakMap)}
                ...
                >
                    <img src={mapImage} alt={`Analyzed Map Revision ${revision}`} className="w-full h-32 object-cover" />
                </button>
            </div>
